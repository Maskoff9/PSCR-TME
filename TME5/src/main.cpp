...
public:
		DrawJob(const Scene::screen_t & screen,

	const Scene & scene,
	Color* pixels,
	int x,
	int y,
	std::vector<Vec3D>& lights): screen(screen), scene(scene), pixels(pixels), x(x), y(y), lights(lights) {}
...
int main () {
	...
	for (int x =0 ; x < scene.getWidth() ; x++) {
		for (int  y = 0 ; y < scene.getHeight() ; y++) {
			pool.submit(new DrawJob(screen, scene, pixels, x, y, lights));
